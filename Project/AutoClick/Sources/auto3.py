...
action_sequence_waiting = RecognitionInfomation(
    ACTION.CLICK, RESULT.NG, END_ACTION.CONTINUE, 4, 20, '../Images/image_waiting/*.png', 4, 0.99, False)
action_sequence_start = RecognitionInfomation(
    ACTION.CLICK, RESULT.OK, END_ACTION.CONTINUE, 2, 0, '../Images/image_start/Click0*.png', 1.8, 0.93, True)
action_sequence_start2 = RecognitionInfomation(
    ACTION.CLICK, RESULT.OK, END_ACTION.CONTINUE, 1, 0, '../Images/image_start/*.png', 1.8, 0.93, True)
action_sequence_main = RecognitionInfomation(
    ACTION.CLICK, RESULT.OK, END_ACTION.CONTINUE, 4, 0, '../Images/image/*.png', 0.5, 0.93, True)
action_sequence_main2 = RecognitionInfomation(
    ACTION.CLICK, RESULT.NG, END_ACTION.CONTINUE, 3, 100, '../Images/image/Click02*.png', 0.5, 0.93, True)
...
action_sequence_shutdown = RecognitionInfomation(
    ACTION.CLICK, RESULT.ALL_OK, END_ACTION.CONTINUE, 2, 2, '../Images/image_shutdown/*.png', 5, 0.95, True)
...
def Image_SearchAndMove(image_path, x_offset_dictionary, y_offset_dictionary, recognition_grayscale, recognition_confidence):
    try:
        ...
        log.Log_MessageAdd(message_list, "ImageSearchAndMove(" +
                       image_path + ")" + str(x)+","+str(y))
...
def timecheck_ikusei():
    ...
    check1 = week_day.Check_WithinRangeDay(
        date_time, time1, day_of_weekday1, time2, day_of_weekday2)
    if check1:
    ...
    check2 = week_day.Check_WithinRangeDay(
        date_time, time1, day_of_weekday1, time2, day_of_weekday2)
    if check2:
        message_list.extend(week_day.message_list)
    ...
    week_day.message_list.clear()
    check3 = week_day.Check_WithinRangeDay(
        date_time, time1, day_of_weekday1, time2, day_of_weekday2)
    if check3:
        message_list.extend(week_day.message_list)
    ...
    week_day.message_list.clear()
    check4 = week_day.Check_WithinRangeDay(
        date_time, time1, day_of_weekday1, time2, day_of_weekday2)
    if check4:
        message_list.extend(week_day.message_list)
    ...
    if flag_execute:
        log.Log_MessageAdd(message_list, "time check" +
                       date_time.strftime('%Y 年 %m 月 %d　日　(%A) %H : %M'))
    return flag_execute
...
def information_check():
    ...
    sequence_Infomation0 = RecognitionInfomation(ACTION.CLICK, RESULT.OK, END_ACTION.CONTINUE, 1, 0, '../Images/images/image_Information0/*.png', 0.8, 0.93, True)
    result_action = Images_Action_ByInformation(sequence_Infomation0, x_offset_dictionary, y_offset_dictionary)
    sequence_Infomation1 = RecognitionInfomation(ACTION.CLICK, RESULT.OK, END_ACTION.CONTINUE, 1, 0, '../Images/image_Information1/*.png', 0.8, 0.93, True)
    result_action = Images_Action_ByInformation(sequence_Infomation1, x_offset_dictionary, y_offset_dictionary)
    if result_action:
...
def event2( flag_timecheck):
    if flag_timecheck and datetime.time(00,00,0) <= datetime.datetime.now().time() and  datetime.datetime.now().time() <= datetime.time(20,00,0):       
        action_sequence_event3 = RecognitionInfomation(ACTION.CLICK, RESULT.OK, END_ACTION.CONTINUE, 6, 0, '../Images/image_event2/Click9*.png', 0.8, 0.93, True)
        result_action = Images_Action_ByInformation(action_sequence_event3, x_offset_dictionary, y_offset_dictionary)
    else:
...
def main_process(sequence, server_sequence, flag_timecheck):
    ...
    if Condition_Judge(RESULT.OK, result_file_click):
        ...
        if flag_timecheck == False:
            ...
            flag_execute = timecheck_ikusei()
        if Condition_Judge(RESULT.OK, result_server):
            if flag_login or flag_execute:
                result_start = Images_Action_ByInformation(action_sequence_start, x_offset_dictionary, y_offset_dictionary)  # "プロセス実行", result_waiting
                result_start = Images_Action_ByInformation(action_sequence_start2, x_offset_dictionary, y_offset_dictionary)  # "プロセス実行", result_waiting
                information_check()
                event2(flag_timecheck)
            if flag_login:
                log.Log_MessageAdd(message_list, "time_check:無料ポチポチ実行")
                result_action = Images_Action_ByInformation(action_sequence_main, x_offset_dictionary, y_offset_dictionary)  # "プロセス実行", result_waiting
                action_sequence_main3 = RecognitionInfomation(ACTION.CLICK, RESULT.NG, END_ACTION.CONTINUE, 3, 5, '../Images/image/Click07*.png', 0.2, 0.93, True)
                result_action = Images_Action_ByInformation(action_sequence_main3, x_offset_dictionary, y_offset_dictionary)  # "プロセス実行", result_waiting
                result_action = Images_Action_ByInformation(action_sequence_main2, x_offset_dictionary, y_offset_dictionary)  # "プロセス実行", result_waiting
...
def main():
    ...
    try:
        while 1:
            #Hospot起動
            action_hotspot = RecognitionInfomation(ACTION.CLICK, RESULT.OK, END_ACTION.FOLDER_END_BREAK, 2, 2, '../Images/hotspot/*.png', 1.8, 0.95, True)
            result_action = Images_Action_ByInformation(action_hotspot,x_offset_dictionary,y_offset_dictionary)
            ...
            if Condition_Judge(RESULT.OK, result_action):
                ...
                action_mementomori = RecognitionInfomation(ACTION.CLICK, RESULT.NG, END_ACTION.CONTINUE, 3, 5, '../Images/mementomori/Click1*.png', 1.8, 0.95, True)
                result_action = Images_Action_ByInformation(action_mementomori,x_offset_dictionary,y_offset_dictionary)
            ...
            Images_Action_ByInformation(action_sequence_end, x_offset_dictionary, y_offset_dictionary)  # "終了処理", RESULT.OK
            #Images_Action_ByInformation( action_sequence_shutdown,x_offset_dictionary,y_offset_dictionary)#"Shutdown処理", RESULT.ALL_OK#
