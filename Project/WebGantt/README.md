...
- **`gist`（既定・推奨）**：GitHub の Fine-grained PAT（Gists: Read and write 権限）でログイン。
...
2. 利用者は GitHub で **Fine-grained personal access token** を作成
   （Settings → Developer settings → Fine-grained tokens、**Account permissions → Gists: Read and write**）。
